...
async function bootstrap() {
  ...
  try {
    ...
    // All roles get the same permissions
    const allPermissionIds = updatedPermissionsInDb.map((p) => ({ id: p.id }))
    await Promise.all([
      updateRole(allPermissionIds, Role.ADMIN),
      updateRole(allPermissionIds, Role.LABEL),
      updateRole(allPermissionIds, Role.LISTENER),
    ])
